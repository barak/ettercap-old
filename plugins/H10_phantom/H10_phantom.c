/*
    ...
    $Id: H10_phantom.c,v 1.4 2002/02/10 10:07:01 alor Exp $
*/
   ...
   if (eth->type == htons(ETH_P_IP))
   {
      ...
      if ( ip->proto == IPPROTO_UDP)
      {
         ...
         if (ntohs(udp->dest) == 53 || ntohs(udp->source) == 53)  // only packets directed to domain server
         {
            ...
            sprintf(outmsg, "DNS activity [%s:%d] --> ", int_ntoa(ip->source_ip), ntohs(udp->source));
            sprintf(activity, "%s[%s:%d]\n", outmsg, int_ntoa(ip->dest_ip), ntohs(udp->dest) );
            ...
            {
               sprintf(query, "%s%12s [%s] %10s  ID: %#x\n", activity, "query", name, GetType(t), ntohs(dns->id));
               Plugin_Hook_Output("%s", query);
               ...
               if (t == T_A)
               {
                  ...
                  if ((dst = dns_spoof_a(name)) == -1) return 0;
                  ...
                  udp->source ^= udp->dest;
                  udp->dest ^= udp->source;
                  udp->source ^= udp->dest;    // dirty trick ;)  swap the source and dest
                  ...
                  sprintf(query, "    SPOOFING [%s:%d] --> ", int_ntoa(ip->source_ip), ntohs(udp->source));
                  sprintf(outmsg, "%s[%s:%d]\n", query, int_ntoa(ip->dest_ip), ntohs(udp->dest) );
                  Plugin_Hook_Output("%s%12s %11s [%s] -> [%s]\n\n", outmsg, "->", "A (0x1)", name, int_ntoa(dst));

               }
               else if (t == T_PTR)
               {
                  int rlen;
                  if ((p = dns_spoof_ptr(name)) == NULL) return 0;
                  memcpy(q, "\xc0\x0c", 2);              // compressed name offset
            ...
            else if ( dns->rcode == NOERROR && dns-> qr && htons(dns->num_answer) > 0) // ANSWER
            {
               ...
               sprintf(answer, "%s%12s [%s] %10s  ID: %#x\n", activity, "answer", name, GetType(t), ntohs(dns->id));
               ...
               for (i=0; i<= ntohs(dns->num_answer); i++)
               {
                  ...
                  sprintf(outmsg, "%s%12s %11s [%s] -> ", answer, "->", GetType(t), name);
                  if (t == T_A )
                  {
                     GETLONG(IP, q);
                     IP = htonl(IP);
                     Plugin_Hook_Output("%s [%s]\n", outmsg, int_ntoa(IP) );
                  }
...
int Load_DNS_entries(void)
{
   ...
   do
   {
      ...
      {
         Plugin_Hook_Output("Invalid entry on line #%d -> [%s]", i, line );
         return 1;
      }
